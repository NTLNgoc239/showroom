...
export default {
  ...
  mounted() {
    ...
    if (this.$slots.default && this.$slots.default.length > 1) {
      console.warn(
        "[✋ vue-breakpoints] You may only wrap one element in a <showAt> or <hideAt/> component."
      );
    } else if (!this.$slots.default || this.$slots.default.length < 1) {
      console.warn(
        "[✋ vue-breakpoints] You must have one child inside a <showAt> or <hideAt/> component."
      );
    }
  },
